...
export function CinemaStudio() {
    ...
    generateBtn.className = 'h-[56px] px-8 bg-[#FF4500] text-black rounded-xl font-black text-xs uppercase hover:bg-white transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed';
    ...
    const createActionBtn = (label, primary = false) => {
        ...
        btn.className = primary
            ? 'bg-[#FF4500] text-black px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wide hover:bg-white transition-colors shadow-glow-sm hover:scale-105 active:scale-95'
    ...
    const renderHistory = () => {
        ...
        generationHistory.forEach((entry, idx) => {
            ...
            thumb.className = `relative group/thumb cursor-pointer rounded-lg overflow-hidden border-2 transition-all duration-300 aspect-square ${idx === 0 ? 'border-[#FF4500] shadow-glow-sm' : 'border-white/10 hover:border-white/30'}`;
    ...
    const loadHistoryItem = (entry, thumbElement) => {
        ...
        if (thumbElement) {
            historyList.querySelectorAll('div').forEach(t => {
                t.classList.remove('border-[#FF4500]', 'shadow-glow-sm');
                t.classList.add('border-white/10');
            });
            thumbElement.classList.remove('border-white/10');
            thumbElement.classList.add('border-[#FF4500]', 'shadow-glow-sm');
        }
    };
