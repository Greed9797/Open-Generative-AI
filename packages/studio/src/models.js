...
// Filtered to provider adapters available in this fork for t2i/t2v models.
...
const enumFor = (model, field) => model?.inputs?.[field]?.enum || [];
const hasField = (model, field) => Boolean(model?.inputs?.[field]);
...
export const getAspectRatiosForModel = (id) => enumFor(getModelById(id), 'aspect_ratio');
export const getAspectRatiosForVideoModel = (id) => enumFor(getVideoModelById(id), 'aspect_ratio');
export const getAspectRatiosForI2IModel = (id) => enumFor(getI2IModelById(id), 'aspect_ratio');
export const getAspectRatiosForI2VModel = (id) => enumFor(getI2VModelById(id), 'aspect_ratio');

export const getDurationsForModel = (id) => enumFor(getVideoModelById(id), 'duration');
export const getDurationsForI2VModel = (id) => enumFor(getI2VModelById(id), 'duration');

export const getResolutionsForModel = (id) => enumFor(getModelById(id), 'resolution');
export const getResolutionsForVideoModel = (id) => enumFor(getVideoModelById(id), 'resolution');
export const getResolutionsForI2IModel = (id) => enumFor(getI2IModelById(id), 'resolution');
export const getResolutionsForI2VModel = (id) => enumFor(getI2VModelById(id), 'resolution');
export const getResolutionsForLipSyncModel = (id) => enumFor(getLipSyncModelById(id), 'resolution');

export const getModesForModel = (id) => enumFor(getVideoModelById(id), 'mode');

export const getQualityFieldForModel = (id) => {
  const model = getModelById(id);
  if (hasField(model, 'quality')) return 'quality';
  if (hasField(model, 'resolution')) return 'resolution';
  return null;
};

export const getQualityFieldForI2IModel = (id) => {
  const model = getI2IModelById(id);
  if (hasField(model, 'quality')) return 'quality';
  if (hasField(model, 'resolution')) return 'resolution';
  return null;
};

export const getMaxImagesForI2IModel = (id) => {
  const model = getI2IModelById(id);
  return model?.inputs?.images_list?.maxItems || model?.inputs?.image_urls?.maxItems || 1;
};
