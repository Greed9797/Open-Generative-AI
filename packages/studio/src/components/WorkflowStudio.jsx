  ...
  loading: () => (
    <div className="absolute inset-0 flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="w-12 h-12 border-4 border-white/5 border-t-[#FF4500] rounded-full animate-spin" />
...
function WorkflowCard({ workflow, onClick, activeTab, onRename, onDelete }) {
  ...
  return (
    <div
      ...
      className="group relative aspect-[3/4] rounded-lg overflow-hidden cursor-pointer border border-white/5 bg-[#0a0a0a] transition-all hover:border-[#FF4500]/30 hover:scale-[1.02] shadow-2xl"
    >
      {workflow.thumbnail ? (
        <img
        ...
        <div className="absolute inset-0 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #1a1a1a, #2a2a2a)' }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="1.5"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
        </div>
      )}
          ...
          {showOptions && (
            <div className="absolute top-10 right-0 w-32 bg-[#111] border border-white/10 rounded-lg shadow-2xl py-1 animate-in fade-in zoom-in duration-200">
              <button
                ...
                className="w-full px-4 py-2 text-left text-[11px] font-bold text-white/70 hover:text-[#FF4500] hover:bg-white/5 transition-colors flex items-center gap-2"
              >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4">
        <div className="text-[10px] font-bold text-[#FF4500] uppercase tracking-wider mb-1 opacity-80">
          {workflow.category || "Geral"}
        </div>
        <h3 className="text-sm font-bold text-white truncate group-hover:text-[#FF4500] transition-colors">
          {workflow.name || "Fluxo sem título"}
        </h3>
      </div>
    </div>
  );
...
export default function WorkflowStudio({ apiKey, isHeaderVisible = true, onToggleHeader }) {
  ...
  const [builderTimedOut, setBuilderTimedOut] = useState(false);
      ...
      try {
        ...
        if (results[1].status === 'rejected' || results[2].status === 'rejected') {
          ...
          if (!nodes.length && !def.nodes?.length) {
             setError("Falha ao carregar dados completos do builder. Alguns recursos podem estar desabilitados.");
          }
        }
      } catch (err) {
        console.error("Critical error loading pulse details:", err);
        setError("Erro crítico ao carregar o builder: " + err.message);
        setNodeSchemas([]);
  ...
  const handleCreateWorkflow = useCallback(
    async (fromUrl = false) => {
      try {
        ...
        setError("Falha ao inicializar workflow: " + err.message);
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleDeleteWorkflow = async (wfId) => {
    ...
    try {
      ...
      alert("Falha ao excluir workflow");
    } finally {
  ...
  const handleRenameWorkflow = async (e) => {
    ...
    try {
      ...
      alert("Falha ao renomear workflow");
    }
  };
      ...
      if (urlWorkflowId === "new") {
        ...
        if (found) {
          ...
          handleSelectWorkflow(
            { id: urlWorkflowId, name: "Carregando..." },
            true,
          );
        }
      }
  ...
  // Builder timeout: if builder data hasn't loaded in 8s, show error
  useEffect(() => {
    if (activeSubTab !== "builder") {
      setBuilderTimedOut(false);
      return;
    }
    if (nodeSchemas && workflowDef) return;
    const timer = setTimeout(() => setBuilderTimedOut(true), 8000);
    return () => clearTimeout(timer);
  }, [activeSubTab, nodeSchemas, workflowDef]);
  ...
  useEffect(() => {
    async function loadWorkflows() {
      try {
        // Proteção contra chamadas sem API Key ou chaves "null"/"undefined" vindas do shell
        if (!apiKey || apiKey === "null" || apiKey === "undefined" || apiKey === "") {
          setWorkflows([]);
          if (activeMainTab === "my-workflows") {
            setError("Uma chave MuAPI válida é necessária para visualizar seus workflows pessoais. Verifique suas configurações.");
          }
          setLoading(false);
          return;
        }
        ...
        setError(err.message || "Falha ao carregar a lista de workflows.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleRun = async (e) => {
    ...
    if (!apiKey) {
      setError("Chave de API ausente. Forneça sua chave MuAPI nas configurações.");
      setIsExecuting(false); // Stop execution immediately
      return;
    }
  ...
  if (selectedWorkflow) {
    return (
      <div className="h-full w-full max-w-full flex flex-col bg-[#030303] text-white overflow-x-hidden">
        ...
        {isHeaderVisible ? (
          <div className="flex-shrink-0 h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/40 z-30">
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-md transition-all ${
                      activeSubTab === "playground"
                        ? "bg-[#FF4500] text-black shadow-[0_0_15px_rgba(255,69,0,0.2)]"
            ...
            <div className="flex items-center gap-3">
              <span className="text-[11px] font-black text-[#FF4500] uppercase tracking-widest">
          ...
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-4 px-4 py-2 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full shadow-2xl animate-fade-in-down">
            <button
               ...
               title="Voltar para Todos os Workflows"
               type="button"
            >
            ...
            <div className="flex bg-white/5 p-1 rounded-lg">
               <button
                 ...
                 className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all ${
                   activeSubTab === "playground" ? "bg-[#FF4500] text-black" : "text-white/40"
                 }`}
               >
                 Play
               </button>
               <button
                 ...
                 type="button"
                 className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all ${
                   activeSubTab === "builder" ? "bg-[#FF4500] text-black" : "text-white/40"
                 }`}
               >
                 Builder
               </button>
        ...
        <div className="flex-1 min-h-0 min-w-0 overflow-hidden flex flex-col lg:flex-row">
          {activeSubTab === "playground" ? (
            <>
              {/* Controls Panel */}
                ...
                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                  <form onSubmit={handleRun} className="space-y-6">
                      ...
                      <div className="space-y-4">
                        {!inputSchema && !loading && (
                          <p className="text-xs text-white/30 leading-relaxed">
                            Este workflow não tem parâmetros configuráveis.<br />
                            Clique em Run Workflow para executar com as configurações padrão.
                          </p>
                        )}
                        {inputSchema &&
                              ...
                              <div key={key} className="space-y-2">
                                ...
                                {prop.type === "string" && !prop.enum ? (
                                  <textarea
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-[#FF4500]/50 transition-colors min-h-[80px] resize-none"
                                    placeholder={
                                      prop.description || `Inserir ${key}...`
                                    }
                                  />
                                ) : prop.enum ? (
                                  <select
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-[#FF4500]/50 transition-colors"
                                  >
                                    {prop.enum.map((opt) => (
                                      <option
                                        key={opt}
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-[#FF4500] text-black text-xs font-black uppercase tracking-[0.2em] rounded-xl hover:bg-white transition-all transform hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:grayscale shadow-[0_0_30px_rgba(255,69,0,0.15)] flex items-center justify-center gap-3 mt-8"
                    >
                      {isExecuting ? (
                        <>
                    ...
                    {!selectedWorkflow.id && (
                      <p className="text-[10px] text-white/30 text-center mt-4">
                        Salve seu workflow primeiro para habilitar a execução.
                      </p>
                    )}
                  </form>
                </div>
              ...
              {/* Preview Panel */}
              <div className="flex-1 overflow-y-auto overflow-x-hidden p-8 lg:p-12 bg-[#050505] flex items-center justify-center min-h-[500px] min-w-0">
                {error && (
                ...
                {!isExecuting && !result && !error && (
                  <div className="flex flex-col items-center gap-6 opacity-40">
                    ...
                    <p className="text-xs text-white/40 max-w-[200px] mx-auto text-center font-medium">
                      Configure os parâmetros e execute o workflow para ver os resultados.
                    </p>
                  </div>
                )}
                ...
                {isExecuting && (
                  <div className="flex flex-col items-center gap-6 animate-fade-in">
                    <div className="relative">
                      <div className="w-24 h-24 border-[3px] border-white/5 border-t-[#FF4500] rounded-full animate-spin shadow-[0_0_40px_rgba(255,69,0,0.1)]" />
                      <div className="absolute inset-0 flex items-center justify-center text-[#FF4500]">
                        <svg
                          width="32"
                          height="32"
                          viewBox="0 0 24 24"
                    ...
                    <div className="text-center space-y-2">
                      <div className="text-[10px] font-black text-[#FF4500] uppercase tracking-[0.3em] animate-pulse">
                        Executando Pipeline
                      </div>
                      <div className="text-[13px] text-white/40 font-medium">
                        Processando nós e gerando assets...
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="w-full max-w-4xl space-y-8 animate-fade-in-up">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {result.outputs?.map((out, idx) => (
                        <div
                          key={idx}
                          className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-[#FF4500]/30 transition-all shadow-2xl"
                        >
                          {out.type === "image_url" ? (
                            <img
                              src={out.value}
                            ...
                            <div className="p-6 min-h-[200px] flex items-center justify-center italic text-white/60 break-all">
                              {out.value}
                            </div>
                          )}
                          ...
                          <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform">
                            <div className="flex items-center justify-between">
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center hover:bg-[#FF4500] hover:text-black transition-colors"
                              >
                                <svg
                                  width="14"
                                  height="14"
              ...
              {nodeSchemas && workflowDef ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="flex flex-col items-center gap-4 max-w-sm text-center px-6">
                    <div className="text-[11px] font-black text-white/30 uppercase tracking-widest">Builder indisponível</div>
                    <p className="text-xs text-white/40 leading-relaxed">
                      Use o Playground para executar workflows.
                    </p>
                    <button
                      type="button"
                      onClick={() => setActiveSubTab("playground")}
                      className="px-4 py-2 bg-[#FF4500] text-black text-[10px] font-black uppercase tracking-widest rounded-lg"
                    >
                      Abrir Playground
                    </button>
                  </div>
  ...
  return (
    <div className="h-full w-full flex flex-col p-8 overflow-y-auto overflow-x-hidden custom-scrollbar">
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="text-white/40 text-sm font-medium">
                Crie e gerencie seus pipelines assíncronos de processamento de IA
              </p>
            </div>
            <button
              onClick={() => handleCreateWorkflow()}
              className="px-6 py-3 bg-[#FF4500] text-black text-xs font-black uppercase tracking-widest rounded-lg hover:bg-white transition-all transform hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(255,69,0,0.3)] flex items-center gap-2"
            >
              <svg
                width="14"
                height="14"
            ...
            {!loading && workflows.length === 0 && (
              <div className="col-span-full py-24 text-center border-2 border-dashed border-white/5 rounded-2xl bg-white/[0.02]">
                <div className="text-white/20 text-sm font-medium italic">
                  Nenhum workflow encontrado nesta seção.
                </div>
              </div>
            )}
          </div>
      ...
      {renamingWorkflow && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          >
            <h3 className="text-xl font-bold text-white mb-2">Renomear Workflow</h3>
            <p className="text-white/40 text-sm mb-6">Digite um novo nome descritivo para seu pipeline.</p>
            
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#FF4500] uppercase tracking-widest">Nome do Workflow</label>
                <input
                  autoFocus
                  type="text"
                  value={newWorkflowName}
                  onChange={(e) => setNewWorkflowName(e.target.value)}
                  placeholder="ex: Fluxo de Vídeo Cinemático"
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#FF4500]/50 transition-colors"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-[#FF4500] text-black px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-white transition-all transform hover:scale-105 active:scale-95"
                >
                  Salvar Nome
                </button>
              </div>
