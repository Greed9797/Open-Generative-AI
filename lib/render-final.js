...
function buildHtml(job) {
  ...
  const clips = job.segments.map((segment, index) => {
    const start = index * 8;
    const isImage = /\.(png|jpe?g|webp)(\?|$)/i.test(segment.finalClipUrl);
    if (isImage) {
      return `<img class="clip" data-start="${start}" data-duration="8" data-track-index="0" src="${segment.finalClipUrl}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;" />`;
    }
    return `<video class="clip" data-start="${start}" data-duration="8" data-track-index="0" src="${segment.finalClipUrl}" muted playsinline style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;"></video>`;
  }).join('\n');
  return `<!doctype html>
<html>
<head><meta charset="utf-8"><style>html,body{margin:0;background:#000;overflow:hidden}</style></head>
<body>
<div id="root" data-composition-id="${compositionId}" data-start="0" data-width="1920" data-height="1080" style="position:relative;width:1920px;height:1080px;background:#000;overflow:hidden;">
${clips}
</div>
</body>
</html>`;
...
export async function renderFinal(jobId) {
  ...
  try {
    ...
    await execFileAsync(
      'npx',
      ['hyperframes', 'render', '--input', htmlPath, '--output', outputPath, '--fps', '30', '--quality', 'high'],
