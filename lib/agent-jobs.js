...
function toDbRow(job) {
  return {
    ...
    cinematography_plan: job.cinematographyPlan || null,
    vision_briefing: job.visionBriefing || null,
    preprocessing_results: job.preprocessingResults || [],
    postprocessing_results: job.postprocessingResults || [],
...
function fromDbRow(row) {
  ...
  return {
    ...
    cinematographyPlan: row.cinematography_plan,
    visionBriefing: row.vision_briefing,
    preprocessingResults: row.preprocessing_results || [],
    postprocessingResults: row.postprocessing_results || [],
...
export async function getPublicJob(id, userId) {
  if (!isSafeJobId(id) || !userId) return null;
  const memJob = jobs.get(id);
  if (memJob) return memJob.userId === userId ? sanitizeJob(memJob) : null;

  const diskJob = loadJobFromDisk(id);
  if (diskJob) {
    jobs.set(id, diskJob);
    return diskJob.userId === userId ? sanitizeJob(diskJob) : null;
  }
