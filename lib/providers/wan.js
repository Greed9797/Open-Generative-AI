...
function mapModel(model) {
  // Map model IDs to DashScope model names
...
export async function submit(payload, apiKey) {
  const isImage = payload.model?.includes('image') || payload.model?.includes('t2i');
  const path = isImage
  ...
  const body = {
    ...
    parameters: {
      size: mapSize(payload.aspect_ratio),
    },
  };
  if (!isImage && payload.duration) body.parameters.duration = Number(payload.duration);
